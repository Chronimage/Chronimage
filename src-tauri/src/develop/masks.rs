...
pub fn apply_mask_layers(
    ...
    for mask in masks {
        ...
        blend_masked(&mut base, &adjusted, &alpha, &mask.mode);
    }

    Ok(crate::develop::pipeline::unit_buf_to_rgb(w, h, &base, img))
}
...
fn rasterize_payload(payload: &serde_json::Value, w: usize, h: usize) -> AppResult<Vec<f32>> {
    ...
    match kind {
        "full" | "all" | "prompt" | "subject" | "sky" | "object" | "person" | "background"
        ...
        "radial_gradient" | "radial" => {
            ...
            for y in 0..h {
                ...
                for x in 0..w {
                    ...
                    let d = ((xn - cx).powi(2) + (yn - cy).powi(2)).sqrt();
                    alpha[y * w + x] = if d <= inner {
                        1.0
                    } else {
                        (1.0 - ((d - inner) / (radius - inner).max(1e-6))).clamp(0.0, 1.0)
                    };
                }
            }
        }
...
fn blend_masked(base: &mut [[f32; 3]], adjusted: &[[f32; 3]], alpha: &[f32], mode: &str) {
